---
title: "Personal Learning: The Effects of Class Size and School Setting on Math Performance in STAR Study"
...
The STAR data set focused on the benefits of classroom sizes for
students and was a study conducted in 1985 to 1989. The study focused on
students in kindergarten and first through third grade, recording their
math and reading scores those four years. The target students in this
study were in seventy nine different schools throughout Tennessee in
kindergarten, who would be tracked for their adolescent academic lives.
These schools used in the study were of a variety of types including
inner city schools, rural schools, urban school and suburban schools.
Any appropriate school within Tennessee could join the study, allowing
for a broader scope in what schools were included.

With 7,000 students per grade level, these students were each randomly
assigned to one of three classroom types: small classrooms (15-17
students), regular classrooms (22-25 students) and regular classrooms
with an aide to assist the teacher. Due to the variety of schools and
locations used the study believes it has allowed for a reasonable
representation of different incomes and ethnicity among the students
(Achilles, 6). Similarly to the students teachers were randomly assigned
classroom types to avoid any issues with teachers specializing in
teaching particular sizes of classrooms.
...
-   `g1urban`: The type of school the student is enrolled in, with
    levels `inner city`, `suburban`, `rural` and `urban`.
...
kable(c(Mean = mean(STAR$g1tmathss, na.rm = T),  #Means of math scores
        Stddev = sd(STAR$g1tmathss, na.rm = T)),
      col.names = 'Summary stats',
      caption = 'Table 1: Mean and Standard Deviation of 1st Grade Math Scores')
  
kable(c(q25 = quantile(STAR$g1tmathss, probs = 0.25, na.rm = T), 
  q50 = quantile(STAR$g1tmathss, probs = 0.50, na.rm = T), 
  q75 = quantile(STAR$g1tmathss, probs = 0.75, na.rm = T)),
  col.names = 'Quartiles',
  caption = 'Table 2: Quartiles of 1st Grade Math Scores') 
...
caption = 'Table 3: Missing Values Summary')

```

## Analysis of Missing Values

Looking at the math scores it can be noted that 43.12% of these values
are missing. However upon investigating, a majority of these missing
values are from students who have left the STAR program at this point.
Removing these non-STAR students from our data we can find that this
reduces the number of missing values to 3.38%, a much more manageable
proportion.
...
rownames(na_df) = c('Percentage of Missing Values', 'Percentage of missing values of enrolled stuents')
kable(na_df, caption = 'Table 4: Missing Values for STAR and non-STAR students')
```
...
For this particular school however these missing values only make up
22.40% of the students here. Since much of the data from this school
remains we will choose to keep this school in the study rather than
dropping all values from it.

Due to these findings we will chose to drop any student who has a
missing value for their 1st grade math score as this will eliminate all
non-STAR students and only a minuscule fraction of STAR students who are
missing math scores.
...
hist(na_star$na_count[which(is.na(na_star$g1tmathss))], breaks = 1:30,
     ...
     main = 'Figure 1: Frequency of NA Values among Schools')

#Notice one school has 28 missing, out of how many total? What %%?
28 / length(STAR$g1schid[which(STAR$g1schid == na_star$g1schid[which(na_star$na_count == 28)])])

```

## Aggregated Teacher Statistic

Due of the high correlation between students in the same class we
instead chose to use the median as a summary statistic for each teacher.
Similar to how all students in a class are highly correlated since the
way a teacher educates is likely the same between different classes we
will calculate the median for each teacher to avoid potential
correlation. The median was chosen as our statistic as previous
investigations showed using the mean would lead us to a model that
deviated from our key model assumptions.
...
## Descriptive statistics for each teacher:
...
kable(teacher_dat, caption = 'Table 5: Summary Statistics for Median Scores per Teacher')
...
summary statistics for each class type can be seen in Table 6, which
...
kable(classdat_df, caption = 'Table 6: Summary Statistics per Class Size')
...
Below we can see the summary statistics for each school system can be
seen in Table 7. As seen in Figure 4 we have compared the median math
scores per individual school, with coloring per school setting to
differentiate between them. Figure 5 displays a boxplot of median math
scores per school setting, where we can see that inner city schools have
lower medians than the other three settings which are approximately
equal.
...
kable(school_setting_df, caption = 'Table 7: Summary Statistics per School Setting')


#Boxplot of median score per school
ggplot(data = STARt, aes(g1schid, medn_score, color = g1surban)) + 
  geom_boxplot() + 
...
ggplot(data = STARt, aes(g1surban, medn_score, color = g1surban)) + 
  ...
  xlab('1st grade School') +
  ylab('Average Test score Year 1') +
  ggtitle('Figure 5: Median Combined Test Score per School Setting Year 1') 
...
class type and school setting as our independent variables and the
median math score per teacher as our response variable. School setting
was chosen over individual schools due to issues with unequal variance,
with school setting allowing us to still examine the effects of
different schools.
...
With our fixed effects model we will have the following assumptions and
constraints to keep in mind:

-   $\sum \alpha_i = 0$

-   $\{\epsilon_{ijk}\} \sim \text{ iid } N(0, \sigma^2)$

-   $\sum_i \alpha_i = \sum_j \beta_j = 0$

-   $\sum^a_{i=1} (\alpha\beta)_{ij} = 0, \forall i$

-   $\sum^b_{j=1} (\alpha\beta)_{ij} = 0, \forall j$
...
As we can our tests return a p-value of 8.83e-05 and 1.36e-15
respectively, indicating that both factors of school setting and
classroom have main effects. Note that this test assumes normality from
the model, which we will discuss further in the sensitivity section of
this report.
...
Via the F-test we report a p-value of 0.3725, indicating that we fail to
...
STAR_medn_aov = aov(medn_score ~ g1classtype + as.factor(g1surban), data = STARt)
summary(STAR_medn_aov)

#Should we drop interaction term?
full_model = aov(medn_score ~ g1classtype * as.factor(g1schid), data = STARt)
red_model = aov(medn_score ~ g1classtype + as.factor(g1schid), data = STARt) 
anova(red_model, full_model)
```

## Unequal Medians

Using the Kruskal-Wallis test at significance level 0.05 we can test the
following hypothesis:

$$
H_0: \text{Medians between all groups are equal.} \\
H_1: \text{Not all medians are equal.}
$$

We perform the Kurskal-Wallis test for both class type and school type.
For class type we found that there is a significant difference in the
...
$$
H_0: \text{There is no difference in median math scores between class types} \\
H_1: \text{There is a difference in median math scores between class types}
$$
...
```{r, warning=F, error=F, message=F, echo = F}
leveneTest(medn_score ~ g1classtype * as.factor(g1surban), STARt)
```
...
```{r, warning=F, error=F, message=F, echo = F}
plot(cooks.distance(STAR_medn_aov), main = 'Figure 12: Cooks Distance for Median Model')
```

# Discussion

In this project we have explored the effect of classroom size on the
test scores of students in relation to the school setting they are
taught in. Student scores were aggregated into median scores per teacher
to avoid issues of correlation. We fit a fixed effect model, using a
variety of parametric and non-parametric tools to examine the data.

With this work we have concluded that both class type and school setting
have statistically significant main effects on the median math test
scores of students. As well we can conclude that the median score of
students taught in a small classroom setting score higher on the math
tests than students taught in other classroom sizes.
