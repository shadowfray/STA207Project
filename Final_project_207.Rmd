...
kable(g1smath_stats, caption = 'Table XX: Summary statistics of 1st grade math scores')
...
ggplot(tot_star_df, aes(x="", y=perc, fill = group)) +
  ...
  xlab('') + ylab('') +
  ggtitle('Figure XX: Proportion of Missing First Grade Math Scores') +
...
kable(na_df, caption = 'Table XX: Missing Values for STAR and non-STAR students')
```
...
missing values (Table XX), we have reduced the number of missing points
...
program in first grade. As seen in Figure AA, few schools are missing
...
quartile of 515 and upper quartile of 557. As seen in Figure 2 the
...
compared to the other two class types (Figure XX). Actual testing will
...
kable(classdat_df, caption = 'Table XX: Summary Statistics per Class Size')

ggplot(data = STARt, aes(g1classtype, medn_score)) + 
  geom_boxplot() +
  xlab('Class Type') +
  ylab('Average Test score Year 1') +
  ggtitle('Figure XX: Median combined test Score per class type')
```

### School Urbanicity: Summary Statistics

Below we can see the summary statistics for the urbanicity of each
school can be seen in Table XX. As seen in Figure XX we have compared
the median math scores per individual school, with coloring per school
urbanicity to differentiate between them. Figure XX displays a boxplot
of median math scores per urbanicity level, where we can see that inner
...
kable(school_setting_df, caption = 'Table XX: Summary Statistics per School Setting')
...
ggplot(class_schid_tbl, aes(x = Var2, y = Freq, fill = Var1)) +
  ...
  ylab('') +
  ggtitle('Figure XY:Treatments of School ID and Class Size') +
...
ggplot(class_urb_tbl, aes(x = Var2, y = Freq, fill = Var1)) +
  ...
  xlab('Invidual Schools') +
  ylab('') +
  ggtitle('Figure XZ: Treatments of Urbanicity and Class Size') +
  scale_fill_manual(values=c("#bae4bc", "#7bccc4", "#43a2ca")) +
  theme_minimal() 
```
...
for class type and individual schools. However, we have seen (Figure XX)
that to ensure that all treatments are represented in our model,
urbanicity will need to surplant the use of school ID. Because each
school belongs to a single type of urbanicity there exists a straight
forward mapping between individual schools and the level of their
urbanicity. Using the urbanicity of the school, readers of this report
may be able to gauge how effective class size can be for their specific
school.
...
Via the F-test we report a p-value of 0.22, indicating that we reject
the null hypothesis. As such there is nointeraction between classroom
type and school urbanicity. That is, school urbanicity and class size
cannot work together under certain combinations to have unique effects
on math scores.
...
anova(STARaov.m1, STARaov.m2) 
anova(STARaov.m2, STARaov.m3) 
...
plot. The main effects of class type can be seen in Figure XX, where it
...
suspicions drawn from the boxplots earlier (Figure XX). This will be of
particular note when answering our secondary question below. In Figure
XX we can see the main effects of the school setting, with a Inner City
...
(Figure XX), allowing us to see that there is little interaction between
...
plotmeans(medn_score ~ g1classtype, data = STARt,
          main = 'Figure XX: Main Effects of Class Type')

plotmeans(medn_score ~ g1surban, data = STARt, 
          main = 'Figure XX: Main Effects of School Setting')
```
...
(Figure XX, Figure XX), there is little indication of non-normality, and
...
Figure XY, which is slightly skewed right. Because this skewness is not
...
plot(STAR_medn_aov$fitted.values, STAR_medn_aov$residuals, 
     xlab = 'Fitted Values',
     main = 'Figure XX: Median Model Residuals vs Fitted plot')

ggplot(data = STAR_medn_aov, aes(sample = residuals(STAR_medn_aov))) +
  stat_qq() +
  stat_qq_line() +
  ggtitle('Figure XX: QQplot of Median Model')
...
hist(residuals(aov(medn_score ~ g1classtype * g1surban, data = STARt)), main = 'Figure XY: Histogram of Interaction Model', xlab = 'Residuals')
```
...
g1urb_model = aov(medn_score ~ g1classtype + g1surban, g1dat)
summary(g1urb_model)
hist(residuals(g1urb_model), 
     xlab = 'Residuals',
     main = 'Figure XX: Residuals of First Grade STAR')
...
our normality assumption. However as seen in Figure XX we once again
