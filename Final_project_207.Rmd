...
To ensure that all class types are represented it should be ensured that
all schools included in our investigation have all class types present.
Unfortunately there are four schools missing at least one class type and
will be dropped from our data, specifically schools 244728, 244736,
244796 and 244839.
...
tot_star_df = data.frame(group = c('Missing Value in STAR', 'Existing Values in STAR', 'Missing Values not in STAR'),
  perc = c(tot_star_na, 1 - tot_star_na - tot_raw_na, tot_raw_na))
...
hist(na_star$na_count[which(is.na(na_star$g1tmathss))], breaks = 1:30,
     ...
     main = 'Figure 1: Frequency of missing values among Schools')

#Notice one school has 28 missing, out of how many total? What %%?
miss28_proportion = 28 / length(STAR$g1schid[which(STAR$g1schid == na_star$g1schid[which(na_star$na_count == 28)])])
...
For the class type compared to the median score we obtained a
